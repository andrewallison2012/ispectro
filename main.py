    ...
    def run(self):
        ...
        while self.serial_running:
            ...
            if (np.array_equal(last_line, line) == False) and (line[8] == 1.0):
                ...
                print(f'while loop {self.port_name} {self.buad_rate}')
...
class MainWindow(QMainWindow, Ui_MainWindow):
    request_signal = pyqtSignal()
    request_update_view_and_view2_signal = pyqtSignal()
    update_view_and_view2_signal = pyqtSignal()
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.subject_id_lineEdit.editingFinished.connect(self.file_path_to_save)
        ...
        self.view_and_view2_thread_timer.timeout.connect(self.view_and_view2_thread)
    ...
    def file_to_save(self):
        self.current_file = self.lineEdit_data_file_name.text()
        self.lineEdit_data_file_name.setText(self.current_file)

    def file_path_to_save(self):
        self.current_file_path = self.subject_id_lineEdit.text()
        self.subject_id_lineEdit.setText(self.current_file_path)

    def regression(self):
        self.csv_file_path = self.lineEdit_data_file_name.text()
        try:
            cm.Canvas(self.csv_file_path)
        except:
    ...
    def save_data(self):
        ...
        self.raw_data = self.raw_data[np.logical_not(self.raw_data[:, 0] == 0)]
        ...
        self.calibrated_data = self.calibrated_data[np.logical_not(self.calibrated_data[:, 0] == 0)]
    ...
    def connect(self):
        ...
        self.groupBox_2.setEnabled(True)
        self.disconnect_pushButton.setEnabled(True)
        self.disconnect_pushButton.show()
        self.connect_pushButton.setEnabled(False)
        self.connect_pushButton.hide()
    ...
    def update_view_and_view2(self):
        ...
        try:
            ...
            print(f'missed update_view_and_view2')


        self.update_view_and_view2_signal.emit()

    def update_view3(self):
        try:
            ...
            print(f'update_view3')

    def update_view4(self):

        try:
            self.init1.setData(x=self.raw_data[:, 0] / 1000, y=np.sqrt((self.raw_data[:, 1] ** 2) + (self.raw_data[:, 2] ** 2)), size=5, pen=(255, 0, 0))
            self.init2.setData(x=self.raw_data[:, 0] / 1000, y=np.sqrt((self.raw_data[:, 3] ** 2) + (self.raw_data[:, 4] ** 2)), size=5, pen=(0, 255, 0))
            self.init3.setData(x=self.raw_data[:, 0] / 1000, y=np.sqrt((self.raw_data[:, 5] ** 2) + (self.raw_data[:, 6] ** 2)), size=5, pen=(0, 0, 255))
        except:
            print(f'update_view4')
